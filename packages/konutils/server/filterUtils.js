	...
	switch (condition.value) {
		...
		case '$groups':
			...
			if ((req.user.groups != null) && _.isArray(req.user.groups)) {
				for (group of Array.from(req.user.groups)) { groups.push(group._id); }
			}
			return groups;
			break;
		case '$allgroups':
...
filterUtils.parseFilterCondition = function(condition, metaObject, req, invert) {
	if (!_.isString(condition.term) || (validOperators.indexOf(condition.operator) === -1) || (condition.value == null)) {
	...
	if (condition.operator === 'between') {
		if (_.isObject(value)) {
			if (_.isObject(value.greater_or_equals) && _.isString(value.greater_or_equals.$date)) {
				value.greater_or_equals = processValueByType(value.greater_or_equals);
			}

			if (_.isObject(value.less_or_equals) && _.isString(value.less_or_equals.$date)) {
...
filterUtils.parseFilterObject = function(filter, metaObject, req) {
	...
	} else if (_.isObject(filter.conditions) && (Object.keys(filter.conditions).length > 0)) {
...
filterUtils.parseDynamicData = function(filter, keyword, data) {
	...
	const parseConditions = function(condition) {
		if (__guard__(condition != null ? condition.value : undefined, x => x.indexOf(keyword)) !== -1) {
