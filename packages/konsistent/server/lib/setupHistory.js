...
Konsistent.History.setup = function() {
	...
	return Konsistent.tailHandle = Konsistent.History.db.tail(cursorDescription, Meteor.bindEnvironment(function(doc) {
...
Konsistent.History.processOplogItem = function(doc) {
	...
	let action = 'create';
	const data = doc.o;
	let metaName = Konsistent.MetaByCollection[ns[Math.min(2, ns.length - 1)]] || Konsistent.MetaByCollection[`data.${ns[2]}`] || Konsistent.MetaByCollection[ns.slice(1).join('.')];
...
Konsistent.History.saveLastOplogTimestamp = function(ts) {
	...
	const flush = function() {
		...
		try {
			...
			console.log(e);
			return NotifyErrors.notify('SaveLastOplogTimestamp', e({
				query,
				data,
				options
			}));
		...
		for (relationsFromDocumentName in references.relationsFrom) {
			...
			for (relation of Array.from(relations)) {
				...
				if (referencedKeys.length > 0) {
					if (referencesToUpdate[relationsFromDocumentName] == null) { referencesToUpdate[relationsFromDocumentName] = []; }
		...
		for (var referenceDocumentName in referencesToUpdate) {
			...
			result.push((() => {
				...
				for (relation of Array.from(relations)) {
					...
					} else if (((relationLookupMeta.fields[relation.lookup] != null ? relationLookupMeta.fields[relation.lookup].isList : undefined) === true) && _.isArray(record[relation.lookup])) {
					...
					if ((lookupId.length > 0) && (action === 'update') && ((data[relation.lookup] != null ? data[relation.lookup]._id : undefined) != null)) {
						...
						if (historyRecord != null) {
							...
							let historyLookupId = historyRecord.data[relation.lookup] != null ? historyRecord.data[relation.lookup]._id : undefined;
							if (((relationLookupMeta.fields[relation.lookup] != null ? relationLookupMeta.fields[relation.lookup].isList : undefined) === true) && _.isArray(historyRecord.data[relation.lookup])) {
								historyLookupId = [];
								for (value of Array.from(historyRecord.data[relation.lookup])) {
					...
					result1.push(Array.from(lookupId).map((lookupIdItem) =>
	...
	for (var referenceDocumentName in references.from) {
		...
		for (fieldName in fields) {
			...
			if ((field.descriptionFields != null ? field.descriptionFields.length : undefined) > 0) {
				for (key of Array.from(field.descriptionFields)) { keysToUpdate.push(key.split('.')[0]); }
			}

			if ((field.inheritedFields != null ? field.inheritedFields.length : undefined) > 0) {
				for (key of Array.from(field.inheritedFields)) { keysToUpdate.push(key.fieldName.split('.')[0]); }
			}
			...
			if (keysToUpdate.length > 0) {
				if (referencesToUpdate[referenceDocumentName] == null) { referencesToUpdate[referenceDocumentName] = {}; }
		...
		for (var inheritedField of Array.from(field.inheritedFields)) {
			if (['always', 'hierarchy_always'].includes(inheritedField.inherit)) {
				...
				if (inheritedField.inherit === 'hierarchy_always') {
					...
					if (((inheritedMetaField != null ? inheritedMetaField.type : undefined) !== 'lookup') || (inheritedMetaField.isList !== true)) {
					...
					if (record[inheritedField.fieldName] == null) { record[inheritedField.fieldName] = []; }
				...
				if ((inheritedMetaField != null ? inheritedMetaField.type : undefined) === 'lookup') {
					...
					if (((record[inheritedField.fieldName] != null ? record[inheritedField.fieldName]._id : undefined) != null) || ((inheritedMetaField.isList === true) && ((record[inheritedField.fieldName] != null ? record[inheritedField.fieldName].length : undefined) > 0))) {
						...
						if (inheritedMetaField.isList !== true) {
							...
							if (_.isArray(inheritedMetaField.descriptionFields)) {
								if (updateData.$set[inheritedField.fieldName] == null) { updateData.$set[inheritedField.fieldName] = {}; }
							...
							if (_.isArray(inheritedMetaField.inheritedFields)) {
								for (let inheritedMetaFieldItem of Array.from(inheritedMetaField.inheritedFields)) {
...
Konsistent.History.processReverseLookups = function(metaName, id, data, action) {
	...
	return (() => {
		...
		for (fieldName in meta.fields) {
			field = meta.fields[fieldName];
			if ((field.type === 'lookup') && (field.reverseLookup != null)) {var affectedRecordsCount, reverseLookupQuery, reverseLookupUpdate;
...
Konsistent.History.processAlertsForOplogItem = function(metaName, action, _id, data, updatedBy, updatedAt) {
	...
	let users = [];
	if (data._user != null) { users = users.concat(data._user); }
	...
	if (action === 'update') {
		...
		({ code } = updatedRecord);
		if (updatedRecord._user != null) { users = users.concat(updatedRecord._user); }
	}
			...
			if ((Namespace.RocketChat != null ? Namespace.RocketChat.alertWebhook : undefined) != null) {
				...
				result.push((() => {
					...
					for (var url of Array.from(urls)) {
						if (!_.isEmpty(url)) {
							result1.push(HTTP.post(url, { data: alertData }, function(err, response) {
								if (err != null) {
