...
SSR.compileTemplate(
	'resetPassword',
	Assets.getText('templates/email/resetPassword.html')
);
...
Meteor.registerMethod('auth:login', function(request) {
	...
	if (_.isString(geolocation)) {
		...
		return new Meteor.Error(
			'internal-error',
			'O Konecty exige que você habilite a geolocalização do seu navegador.'
		);
	}

	const userRecord = Meteor.users.findOne({
	...
	return {
		...
		user: {
			...
			email: __guard__(
				userRecord.emails != null ? userRecord.emails[0] : undefined,
				x => x.address
			),
			group: userRecord.group,
...
Meteor.registerMethod('auth:info', 'withUser', function(request) {
	...
	const response = {
		...
		user: {
			...
			email: __guard__(
				this.user.emails != null ? this.user.emails[0] : undefined,
				x => x.address
			),
...
Meteor.registerMethod('auth:setPassword', 'withUser', function(request) {
	...
	const userRecord = Meteor.users.findOne({
		$or: [{ _id: userId }, { username: userId }, { 'emails.address': userId }]
	});
...
Meteor.registerMethod(
	'auth:setRandomPasswordAndSendByEmail',
	'withUser',
	function(request) {
		// Map body parameters
		...
		for (let userRecord of Array.from(userRecords)) {
			if (
				...
				errors.push(
					new Meteor.Error(
						'internal-error',
						`Usuário [${userRecord.username}] sem email definido.`
					)
				);
				continue;
			}
			...
			if (
				...
				errors.push(
					new Meteor.Error(
						'internal-error',
						`Permissão negada para alterar a senha do usuário [${
							userRecord.username
						}].`
					)
				);
...
Meteor.registerMethod('auth:setGeolocation', 'withUser', function(request) {
	...
	if (longitude == null || latitude == null) {
		return new Meteor.Error(
			'internal-error',
			'Longitude or Latitude not defined'
		);
	}
