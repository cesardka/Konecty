...
app.get('/rest/auth/loginByUrl/:ns/:sessionId', function(req, res, next) {
	...
	req.params.sessionId = decodeURIComponent(
		req.params.sessionId.replace(/\s/g, '+')
	);
	...
	const cookieMaxAge =
		namespace.sessionExpirationInSeconds != null
			? namespace.sessionExpirationInSeconds
			: 2592000;
	...
	if ((loginResult != null ? loginResult.success : undefined) === true) {
		...
		if (_.isString(ns)) {
			res.set(
				...
				`_authTokenId=${
					loginResult.authId
				}; ${domain} Version=1; Path=/; Max-Age=${cookieMaxAge.toString()};_authTokenNs=${ns}; ${domain} Version=1; Path=/; Max-Age=${cookieMaxAge.toString()}`
			);
		} else {
			res.set(
				'set-cookie',
